...
Before calling `tree.sync(guild=...)`, copy the global command tree into that guild with `tree.copy_global_to(guild=...)`; otherwise the sync succeeds but publishes zero commands.
